...
const executable = join(repositoryRoot, 'dist', 'linux-unpacked', 'bronom')
const profileDirectory = await mkdtemp(join(tmpdir(), 'bronom-packaged-smoke-'))
...
async function waitForServer(port: number, timeoutMs = 15_000): Promise<void> {
  ...
  throw new Error('Packaged Bronom MCP server did not start')
}
...
const applicationEnvironment = {
  ...
  BRONOM_MCP_PORT: String(mcpPort),

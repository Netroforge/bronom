...
try {
  ...
  const page = encodeURIComponent(
    '<!doctype html><title>Bronom MCP Smoke</title><h1>Bronom MCP Smoke</h1><label>Name <input aria-label="Name"></label><button onclick="document.querySelector(\'h1\').textContent=\'Smoke passed\'">Run</button><script>console.error(\'bronom-packaged-debug-marker\')</script>'
  )
  ...
  if (!snapshotText.includes('Bronom MCP Smoke') || !snapshotText.includes('[e1]')) {
  ...
  if (debugReport.isError || !text(debugReport).includes('bronom-packaged-debug-marker')) {

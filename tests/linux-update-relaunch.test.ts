...
describe('Linux update relaunch handoff', () => {
  it('waits for the old single-instance owner before starting the installed executable', () => {
    ...
    scheduleLinuxUpdateRelaunch(4321, '/opt/Bronom/bronom', '/usr/share/applications/Bronom.desktop', spawnProcess)
    ...
    expect(args).toEqual([
      '-c',
      LINUX_UPDATE_RELAUNCH_SCRIPT,
      'bronom-update-relaunch',
      '4321',
      '/opt/Bronom/bronom',
      '/usr/share/applications/Bronom.desktop'
    ])
  ...
  it('uses the persistent AppImage path and otherwise the Electron executable', () => {
    expect(linuxUpdateExecutable({ APPIMAGE: '/apps/Bronom.AppImage' }, '/tmp/.mount/bronom')).toBe('/apps/Bronom.AppImage')
    expect(linuxUpdateExecutable({}, '/opt/Bronom/bronom')).toBe('/opt/Bronom/bronom')
  })
  ...
  it('does not launch the replacement until the old process exits', async () => {
    ...
    const helper = spawn('/bin/sh', [
      '-c',
      LINUX_UPDATE_RELAUNCH_SCRIPT,
      'bronom-update-relaunch-test',
      String(blocker.pid),
      '/usr/bin/true',
      '/missing/Bronom.desktop'
    ], { stdio: 'ignore' })
  ...
  it('rejects unsafe relaunch targets', () => {
    ...
    expect(() => scheduleLinuxUpdateRelaunch(0, '/opt/Bronom/bronom', '/usr/share/applications/Bronom.desktop', spawnProcess)).toThrow('Invalid Bronom process ID')
    expect(() => scheduleLinuxUpdateRelaunch(123, 'relative/bronom', '/usr/share/applications/Bronom.desktop', spawnProcess)).toThrow('absolute executable path')
    expect(() => scheduleLinuxUpdateRelaunch(123, '/opt/Bronom/bronom', 'Bronom.desktop', spawnProcess)).toThrow('absolute desktop-file path')

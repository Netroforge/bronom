...
describe('TabSearchPanel', () => {
  it('owns search rendering and runs the matching tab through its controller boundary', async () => {
    ...
    const previousBronom = Object.getOwnPropertyDescriptor(window, 'bronom')
    Object.defineProperty(window, 'bronom', { configurable: true, value: browser })
    ...
    try {
      ...
      view.unmount()
      if (previousBronom) Object.defineProperty(window, 'bronom', previousBronom)
      else Reflect.deleteProperty(window, 'bronom')
    }
  })
})

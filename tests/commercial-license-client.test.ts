...
describe('CommercialLicenseClient', () => {
  it('activates through the Bronom server and parses the sanitized result', async () => {
    ...
    const client = new CommercialLicenseClient('https://bronom.example/api/creem-license', request as unknown as typeof fetch)
    const result = await client.activate('ABCD-EFGH-IJKL-MNOP', 'Bronom 123456789abc')

    expect(result).toMatchObject({ valid: true, status: 'active', instanceId: 'inst_abcdefgh1234' })
    expect(request).toHaveBeenCalledWith(
      'https://bronom.example/api/creem-license/activate',
      expect.objectContaining({ method: 'POST' })
    )
    const body = JSON.parse(String((request.mock.calls[0]?.[1] as RequestInit).body))
    expect(body).toEqual({ licenseKey: 'ABCD-EFGH-IJKL-MNOP', instanceName: 'Bronom 123456789abc' })
  })
  ...
  it('maps a sanitized service error', async () => {
    ...
    const client = new CommercialLicenseClient('https://bronom.example/api/creem-license', request as unknown as typeof fetch)
    await expect(client.activate('ABCD-EFGH-IJKL-MNOP', 'Bronom 123456789abc')).rejects.toMatchObject({
  ...
  it('allows plain HTTP only for local development', () => {
    ...
    expect(() => new CommercialLicenseClient('http://bronom.example/api/creem-license')).toThrow('must use HTTPS')
  })
})

...
test('switches language live through Settings and persists it across restart', async ({
  ...
  await appWindow.evaluate("window.bronomPanelWindow.open('console')")
  ...
  await expect.poll(() => detached.title()).toBe('Консоль — Bronom')
  await appWindow.evaluate('window.bronomPanelWindow.close()')

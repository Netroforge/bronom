...
test('moves the live MCP listener to a validated available port and rolls back on conflicts', async ({
  ...
  await expect.poll(() => appWindow.evaluate('window.bronom.getState().then((state) => state.mcpUrl)')).toBe(`http://127.0.0.1:${nextPort}/mcp`)
  ...
  const restarted = await launchBronom(profileDirectory)
  try {
    await expect.poll(() => status(nextPort)).toBe(200)
    await expect.poll(() => restarted.window.evaluate('window.bronom.getState().then((state) => state.mcpUrl)')).toBe(`http://127.0.0.1:${nextPort}/mcp`)

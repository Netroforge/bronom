...
test('shows update status beside MCP ready and opens update details without moving the webpage', async ({
  ...
  await expect(appWindow.getByRole('button', { name: 'Open Bronom Home' })).toHaveAttribute('aria-current', 'page')
  ...
  const upToDatePill = appWindow.getByRole('button', { name: /Open software updates: Bronom is up to date/ })
  ...
  await expect(updatePanel).toContainText('Bronom 1.1.1 is available')
...
test('renders formatted release notes and removes unsafe update content', async ({ appWindow, electronApp }) => {
  ...
  await electronApp.evaluate(({ BrowserWindow }) => {
    BrowserWindow.getAllWindows()[0]?.webContents.send('updates:changed', {
      ...
      releaseNotes: `
        ...
        <p><a href="https://github.com/Netroforge/bronom/releases">Release page</a></p>
  ...
  await expect(notes.getByRole('link', { name: 'Release page' })).toHaveAttribute('href', 'https://github.com/Netroforge/bronom/releases')
...
test('offers installation retry after system authorization fails', async ({ appWindow, electronApp }) => {
  ...
  await electronApp.evaluate(({ BrowserWindow }) => {
    BrowserWindow.getAllWindows()[0]?.webContents.send('updates:changed', {
      ...
      message: 'System authorization did not complete. Bronom is still running; try the installation again.'
    })
  })

  await appWindow.getByRole('button', { name: /Open software updates: Update needs attention/ }).click()
  const updatePanel = appWindow.getByRole('region', { name: 'Software update status' })
  await expect(updatePanel).toContainText('Bronom is still running')

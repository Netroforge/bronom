...
test('summarizes and selectively clears browsing data without removing retained profile data', async ({
  ...
  const server = createServer((request, response) => {
    ...
    response.end(`<!doctype html><title>Privacy data fixture</title><main>Persistent data</main><script>
      localStorage.setItem('bronom-privacy-fixture', 'stored');
  ...
  try {
    ...
    await appWindow.evaluate(`window.bronomBookmarks.add(${JSON.stringify(url)}, 'Retained privacy bookmark')`)
    await appWindow.evaluate(`window.bronomPermissions.set(${JSON.stringify(origin)}, 'notifications', 'deny')`)
    ...
    expect(await appWindow.evaluate("window.bronomBrowsingData.siteSummary('file:///tmp/not-a-site').then(() => '').catch((error) => error.message)"))
    ...
    await expect.poll(() => appWindow.evaluate('window.bronomBrowsingData.summary()')).toMatchObject({
    ...
    expect(await appWindow.evaluate('window.bronomBookmarks.list()')).toEqual([
      expect.objectContaining({ url, title: 'Retained privacy bookmark' })
    ])
    expect(await appWindow.evaluate('window.bronomPermissions.list()')).toEqual([
    ...
    await expect.poll(async () => (await dialog.boundingBox())!.height).toBe(dialogHeight)
    expect(await appWindow.evaluate('window.bronomMcp.getState()')).toMatchObject({ paused: false })
    await appWindow.evaluate('window.bronomBrowsingData.clear({ history: false, cookiesAndSiteData: true, cache: false })')
    await expect.poll(() => appWindow.evaluate('window.bronomBrowsingData.summary().then((summary) => summary.cookieCount)')).toBe(0)
    await appWindow.evaluate('window.bronomMcp.setPaused(true)')
    await appWindow.evaluate('window.bronomBrowsingData.clear({ history: false, cookiesAndSiteData: false, cache: true })')
    expect(await appWindow.evaluate('window.bronomMcp.getState()')).toMatchObject({ paused: true })
    await appWindow.evaluate('window.bronomMcp.setPaused(false)')
    expect(await appWindow.evaluate(`window.bronomBrowsingData.clear({ history: false, cookiesAndSiteData: false, cache: false })
      .then(() => '')
      .catch((error) => error.message)`)).toContain('Select at least one type')
    expect(requests).toHaveLength(requestCountBeforeClear + 1)
    expect(await electronApp.evaluate(async ({ webContents }, requestedUrl) => {
      const page = webContents.getAllWebContents().find((contents) => contents.getURL() === requestedUrl)
      return page?.executeJavaScript(`localStorage.getItem('bronom-privacy-fixture')`)
    }, url)).toBeNull()
  } finally {

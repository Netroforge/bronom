...
test('scales Bronom without zooming the active website and persists the choice', async ({
  appWindow,
  electronApp,
  mcpPort,
  profileDirectory
  ...
  await expect.poll(() => electronApp.evaluate(({ BrowserWindow }) =>
    BrowserWindow.getAllWindows().find((window) => window.getTitle() === 'Page tools — Bronom')?.webContents.getZoomFactor()
  )).toBe(1.25)
...
test('keeps MCP running when the window is closed to the tray', async ({
  ...
  expect(await response.json()).toMatchObject({ ok: true, name: 'bronom' })
})

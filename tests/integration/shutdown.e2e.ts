...
test('waits for Default and isolated browser profiles to flush before exiting', async ({
  ...
  try {
    const archivedState = await instance.window.evaluate(`window.bronom.createWorkspace({
    ...
    await instance.window.evaluate(`window.bronom.saveAndCloseTabGroup(${JSON.stringify(archivedWorkspaceId)})`)
    await instance.window.evaluate(`window.bronom.createWorkspace({
    ...
    await instance.app.evaluate(({ app, session }, input) => {
      ...
      marker: markerPath,
      profiles: [
        { label: 'default', partition: 'persist:bronom', delayMs: 200 },
        {
          label: 'active',
          partition: `persist:bronom-workspace-${activeStorageId}`,
          delayMs: 600
        },
        {
          label: 'archived',
          partition: `persist:bronom-workspace-${archivedStorageId}`,
          delayMs: 1_000
        }
      ]
    })

...
export function scheduleLinuxUpdateRelaunch(
  oldPid: number,
  executable: string,
  desktopFile = '/usr/share/applications/Bronom.desktop',
  spawnProcess: SpawnUpdateRelaunch = spawn
): void {
  if (!Number.isSafeInteger(oldPid) || oldPid <= 0) throw new TypeError('Invalid Bronom process ID')
  ...
  const helper = spawnProcess(
    '/bin/sh',
    ['-c', LINUX_UPDATE_RELAUNCH_SCRIPT, 'bronom-update-relaunch', String(oldPid), executable, desktopFile],
    {
      detached: true,
      stdio: 'ignore',
      env: { ...process.env }

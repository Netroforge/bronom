...
function agentGuides(
  ...
  const tokenSetup = tokenPath
    ...
    : ''
  const tokenPlaceholder = tokenPath ? `<paste token from ${tokenPath}>` : '<BRONOM_MCP_TOKEN>'
  ...
  return [
    {
      ...
      code: authenticationDisabled
        ...
        : `${tokenSetup}codex mcp add bronom --url ${endpoint} --bearer-token-env-var BRONOM_MCP_TOKEN`
    },
    {
      id: 'claude-code',
      name: 'Claude Code',
      ...
      code: authenticationDisabled
        ...
        : `${tokenSetup}claude mcp add --transport http --scope user --header "Authorization: Bearer $BRONOM_MCP_TOKEN" bronom ${endpoint}`
    },
    {
      id: 'cursor',
      name: 'Cursor',

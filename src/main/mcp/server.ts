...
export const BROWSER_TOOL_CATALOG: BrowserToolDefinition[] = [
  ...
  { name: 'browser_performance', category: 'Inspection', description: 'Measure local Core Web Vitals plus navigation, resources, long tasks, and bounded Long Animation Frame script attribution for the current visit.' },
...
function createBrowserMcpServer(
  ...
  const server = new McpServer({ name: 'bronom', version })
  ...
  registerGroupTool(
    ...
    tabTool('browser_performance', async ({
      ...
      tabId?: string
      settleMs?: number
    }) => textResult(await manager.performanceReport({ tabId, settleMs })))
  )
  registerGroupTool(
    ...
    'browser_network_wait',
    {
      ...
      inputSchema: {
        ...
        afterRequestId: z.string().min(1).optional()
          .describe('Only match requests captured after this retained Bronom request ID. Cannot be combined with from: future.'),
...
export class McpHttpServer {
  ...
  async start(): Promise<string> {
    ...
    app.get('/healthz', (_request, response) => response.json({ ok: true, name: 'bronom', paused: this.paused }))
    app.all('/mcp', async (request: Request, response: Response) => {
      ...
      if (this.paused) {
        response.status(503).json({ error: 'Bronom is paused by the user. Resume agents from the Bronom window.' })
        return
      }

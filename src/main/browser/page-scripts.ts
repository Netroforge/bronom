export function snapshotScript(maxChars: number): string {
  return `(() => {
    ...
    let refIndex = 0;
    for (const element of document.querySelectorAll('[data-bronom-ref]')) element.removeAttribute('data-bronom-ref');
    const lines = [];
    const add = (line) => {
...
function elementInspectionHelpersSource(): string {
  return `
    const bronomCompact = (value, limit) => String(value || '').replace(/\\s+/g, ' ').trim().slice(0, limit);
    const bronomUnique = (selector) => {
      try { return document.querySelectorAll(selector).length === 1; } catch { return false; }
    };
    const bronomSelectorFor = (element) => {
      if (element.id) {
        const selector = '#' + CSS.escape(element.id);
        if (bronomUnique(selector)) return selector;
      }
    ...
    const bronomSafeAttributes = (element) => {
      ...
      return names.flatMap((name) => {
        ...
        return [{ name, value: bronomCompact(value, 300) }];
      });
    };
    const bronomNumber = (value) => {
      const parsed = Number.parseFloat(value);
      return Number.isFinite(parsed) ? Math.round(parsed * 100) / 100 : 0;
    };
    const bronomEdges = (style, prefix, suffix = '') => ({
      top: bronomNumber(style[prefix + 'Top' + suffix]),
      right: bronomNumber(style[prefix + 'Right' + suffix]),
      bottom: bronomNumber(style[prefix + 'Bottom' + suffix]),
      left: bronomNumber(style[prefix + 'Left' + suffix])
    });
    const bronomRole = (element) => {
    ...
    const bronomAccessibleName = (element) => {
    ...
    const bronomContrast = (foreground, background) => {
      const front = bronomRgb(foreground);
      const back = bronomRgb(background);
      if (!front || !back || front.a !== 1 || back.a !== 1) return undefined;
      const first = bronomLuminance(front);
      const second = bronomLuminance(back);
      return Math.round(((Math.max(first, second) + 0.05) / (Math.min(first, second) + 0.05)) * 100) / 100;
    };
    const bronomInspectElement = (element) => {
      const style = getComputedStyle(element);
      const rect = element.getBoundingClientRect();
      const padding = bronomEdges(style, 'padding');
      const border = bronomEdges(style, 'border', 'Width');
      const formControl = element instanceof HTMLInputElement || element instanceof HTMLTextAreaElement || element instanceof HTMLSelectElement;
      const text = bronomCompact(formControl
      ...
      return {
        selector: bronomSelectorFor(element),
        tag: element.localName || element.tagName.toLowerCase(),
        text,
        attributes: bronomSafeAttributes(element),
        box: {
          x: rect.x,
          y: rect.y,
          width: rect.width,
          ...
          margin: bronomEdges(style, 'margin'),
          border,
          padding
        },
        layout: {
        ...
        typography: {
          ...
          contrastRatio: bronomContrast(style.color, style.backgroundColor)
        },
        accessibility: {
          role: bronomRole(element),
          name: bronomAccessibleName(element),
          focusable,
...
export function elementPickerScript(): string {
  return `(() => new Promise((resolve) => {
    const pickerKey = '__bronomElementPicker';
    const areaPicker = window.__bronomScreenshotArea;
    ...
    const makeLayer = (name, background) => {
      ...
      layer.setAttribute('data-bronom-element-picker', name);
    ...
    label.setAttribute('data-bronom-element-picker', 'label');
    ...
    const position = (element) => {
      const inspection = bronomInspectElement(element);
    ...
    const onClick = (event) => {
      ...
      finish({ canceled: false, inspection: bronomInspectElement(candidate) });
    };
    const nativeInput = (type, x, y) => {
      ...
      if (!(candidate instanceof Element) || candidate.hasAttribute('data-bronom-element-picker')) return false;
      current = candidate;
      position(candidate);
      if (type === 'down') {
...
export function cancelElementPickerScript(): string {
  return `(() => {
    const picker = window.__bronomElementPicker;
...
export function elementInspectionScript(target: { ref?: string; selector?: string }): string {
  return `(() => {
    ...
    const element = target.ref
      ? document.querySelector('[data-bronom-ref="' + CSS.escape(target.ref) + '"]')
    ...
    return bronomInspectElement(element);
  })()`
}

export function screenshotAreaScript(): string {
  return `(() => new Promise((resolve) => {
    const pickerKey = '__bronomScreenshotArea';
    const elementPicker = window.__bronomElementPicker;
    if (elementPicker && typeof elementPicker.cancel === 'function') elementPicker.cancel();
    const previous = window[pickerKey];
    if (previous && typeof previous.cancel === 'function') previous.cancel();

    const shade = document.createElement('div');
    shade.setAttribute('data-bronom-screenshot-area', 'shade');
    ...
    selection.setAttribute('data-bronom-screenshot-area', 'selection');
    ...
    hint.setAttribute('data-bronom-screenshot-area', 'hint');
    ...
    size.setAttribute('data-bronom-screenshot-area', 'size');
...
export function screenshotAreaNativeInputScript(
  ...
  return `(() => {
    const picker = window.__bronomScreenshotArea;
    if (!picker || typeof picker.nativeInput !== 'function') return false;
    const width = Math.max(1, ${JSON.stringify(viewportWidth)});
    const height = Math.max(1, ${JSON.stringify(viewportHeight)});
    return picker.nativeInput(

...
class LocalStorageSurface {
  ...
  constructor(partition: string) {
    ...
    this.webContents.debugger.on('message', (_event, method, params) => {
      ...
      void this.webContents.debugger.sendCommand('Fetch.fulfillRequest', {
        ...
        body: Buffer.from('<!doctype html><meta charset="utf-8"><title>Bronom workspace storage</title>').toString('base64')
      }).then(() => {
  ...
  async loadOrigin(origin: string): Promise<void> {
    ...
    await Promise.all([
      this.webContents.loadURL(`${origin}/.well-known/bronom-workspace-storage-${randomUUID()}`),
      intercepted
    ])
  }

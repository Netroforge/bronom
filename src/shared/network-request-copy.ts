...
function safetySummary(payload: CopyPayload): string {
  ...
  return `Sanitized by Bronom: recognized secret headers, URL credentials, and security-related query values are omitted or redacted; structured body secrets use ${REDACTED_VALUE}, but arbitrary URL paths and body text can remain${omissions.length ? `; ${omissions.join('; ')}` : ''}. Review before sharing or running.`
}

...
export function updateErrorMessage(
  ...
  if (platform === 'linux' && operation === 'install' && /Command pkexec exited with code (126|127)/i.test(message)) {
    return 'System authorization did not complete. Bronom is still running; try the installation again.'
  }

  if (platform === 'linux' && /Cannot read properties of undefined \(reading ['"]info['"]\)/i.test(message)) {
    return 'This release has no update package compatible with this Linux installation. Use the matching package from the Bronom release page.'
  }

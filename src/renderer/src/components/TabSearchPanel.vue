      ...
      <section v-if="filteredSavedTabGroups.length" class="tab-search-section saved-groups" aria-labelledby="saved-groups-title">
        ...
        <div role="list" :aria-label="t('tabSearch.archivedAria')">
          ...
          >
            ...
            <button class="tab-search-restore" type="button" :aria-label="t('tabSearch.restoreWorkspaceAria', { name: group.name })" :title="t('tabSearch.restoreWorkspace')" @click="restoreSavedGroup(group)"><IconRestore aria-hidden="true" /></button>
            <button class="tab-search-close" type="button" :aria-label="t('tabSearch.deleteWorkspaceAria', { name: group.name })" :title="t('tabSearch.deleteWorkspace')" @click="deleteSavedGroup($event, group)"><IconDelete aria-hidden="true" /></button>
          </article>
        </div>
      </section>
      <section v-if="filteredTabs.length" class="tab-search-section" aria-labelledby="open-tabs-title">
        ...
        <div role="list" :aria-label="t('tabSearch.openTabs')">
          ...
          >
            ...
            <button class="tab-search-close" type="button" :aria-label="t('tabSearch.closeTabAria', { title: tab.title || t('tabSearch.newTabTitle') })" :title="t('tabSearch.closeTab')" :disabled="state.allHumanInteractionLocked" data-lock-protected-tab-close @click="closeOpenTab($event, tab.id)"><IconClose aria-hidden="true" /></button>
          </article>
        </div>
      </section>
      <section v-if="filteredClosedTabs.length" class="tab-search-section recently-closed" aria-labelledby="closed-tabs-title">
        ...
        <div role="list" :aria-label="t('accessibility.recentlyClosedTabs')">
          ...
          >
            ...
            <button class="tab-search-restore" type="button" :aria-label="t('tabSearch.restoreAria', { title: tab.title || t('tabSearch.newTabTitle') })" :title="t('tabSearch.restore')" @click="restoreClosedTab(tab)"><IconRestore aria-hidden="true" /></button>
          </article>
        </div>
      </section>
    </div>

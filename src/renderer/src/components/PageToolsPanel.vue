  ...
  >
    <header>
      ...
      <div class="panel-header-actions">
        ...
        <button class="panel-close" type="button" :aria-label="t('shell.pageTools.close')" @click="open = false"><IconClose aria-hidden="true" /></button>
      </div>
    </header>
    <div class="page-tools-content">
      <section aria-labelledby="page-tools-inspect-title">
        ...
        <div class="page-tools-grid">
          <button type="button" :aria-label="activeWebUrl ? t('runtime.tabs.siteStorage', { host: hostname }) : t('runtime.tabs.siteStorageUnavailable')" :disabled="!activeWebUrl" @click="actions.toggleSiteStorage">
          ...
          <button :class="{ complete: Boolean(activeEmulation?.viewport) }" type="button" :aria-label="t('runtime.address.responsive', { status: labels.responsive })" @click="actions.toggleResponsivePreview">
            <IconDevices aria-hidden="true" /><span><strong>{{ t('shell.pageTools.responsive') }}</strong><small>{{ labels.responsive }}</small></span>
          </button>
          <button :class="{ complete: environmentOverrideCount > 0, error: environmentState === 'error', running: environmentState === 'applying' }" type="button" :aria-label="t('runtime.address.environment', { status: labels.environment })" :disabled="environmentState === 'applying'" @click="actions.toggleEnvironment">
          ...
          <button type="button" :aria-label="t('shell.pageTools.openConsole')" @click="actions.toggleConsole"><IconTerminal aria-hidden="true" /><span><strong>{{ t('panels.console') }}</strong><small>{{ t('shell.pageTools.consoleDescription') }}</small></span></button>
          <button type="button" :aria-label="t('shell.pageTools.openNetwork')" @click="actions.toggleNetwork"><IconNetworkCheck aria-hidden="true" /><span><strong>{{ t('panels.network') }}</strong><small>{{ t('shell.pageTools.networkDescription') }}</small></span></button>
          <button :class="{ warning: networkRouteCount > 0 }" type="button" :aria-label="t('runtime.address.conditions', { status: networkRouteCount ? t('network.conditions.active', { count: localNumber(networkRouteCount) }, networkRouteCount) : t('runtime.address.noneActive') })" @click="actions.openRequestConditions">
      ...
      <section aria-labelledby="page-tools-diagnose-title">
        ...
        <div class="page-tools-grid">
          <button :class="{ warning: inspectorIssueCount > 0 }" type="button" :aria-label="t('issues.toolAria', { status: labels.inspectorIssues })" @click="toggleInspectorIssues"><IconWarning aria-hidden="true" /><span><strong>{{ t('panels.issues') }}</strong><small>{{ labels.inspectorIssues }}</small></span></button>
          <button :class="{ complete: securityReport?.state === 'secure', warning: securityReport?.state === 'insecure' || securityReport?.state === 'insecure-broken', error: securityReportState === 'error', running: securityReportState === 'loading' }" type="button" :aria-label="t('securityReport.toolAria', { status: labels.security })" :disabled="securityReportState === 'loading'" @click="toggleSecurityReport">
          ...
          <button :class="{ complete: debugReportState === 'complete' && !debugReportSignalCount, warning: debugReportState === 'complete' && Boolean(debugReportSignalCount), error: debugReportState === 'error', running: debugReportState === 'running' }" type="button" :aria-label="labels.debugReport" :disabled="debugReportState === 'running'" @click="toggleDebugReport">
          ...
          <button :class="{ running: activeTab?.reproRecording?.active }" type="button" :aria-label="t('repro.toolAria', { status: labels.repro })" @click="toggleReproRecorder"><IconRecord aria-hidden="true" /><span><strong>{{ t('panels.reproRecorder') }}</strong><small>{{ labels.repro }}</small></span></button>
          <button :class="{ running: activeTab?.domChangesRecording?.active }" type="button" :aria-label="t('domChanges.toolAria', { status: labels.domChanges })" @click="toggleDomChanges"><IconAccountTree aria-hidden="true" /><span><strong>{{ t('panels.domChanges') }}</strong><small>{{ labels.domChanges }}</small></span></button>
      ...
      <section aria-labelledby="page-tools-audit-title">
        ...
        <div class="page-tools-grid">
          ...
          <button :class="{ error: performanceState === 'error', running: performanceState === 'running' }" type="button" :aria-label="labels.performance" :disabled="performanceState === 'running'" @click="togglePerformanceReport"><IconProgress v-if="performanceState === 'running'" class="state-spinner" aria-hidden="true" /><IconError v-else-if="performanceState === 'error'" aria-hidden="true" /><IconMonitoring v-else aria-hidden="true" /><span><strong>{{ t('panels.performance') }}</strong><small>{{ labels.performance }}</small></span></button>
          <button :class="{ warning: Boolean(designOverviewReport?.summary.contrastIssueCount), error: designOverviewState === 'error', running: designOverviewState === 'loading' }" type="button" :aria-label="t('designOverview.toolAria', { status: labels.designOverview })" :disabled="designOverviewState === 'loading'" @click="toggleDesignOverview"><IconProgress v-if="designOverviewState === 'loading'" class="state-spinner" aria-hidden="true" /><IconPalette v-else aria-hidden="true" /><span><strong>{{ t('panels.designOverview') }}</strong><small>{{ labels.designOverview }}</small></span></button>
          <button :class="{ warning: Boolean(pageMetadataReport?.issues.some((issue) => issue.severity !== 'info')), error: pageMetadataState === 'error', running: pageMetadataState === 'loading' }" type="button" :aria-label="t('pageMetadata.toolAria', { status: labels.pageMetadata })" :disabled="pageMetadataState === 'loading'" @click="togglePageMetadata"><IconProgress v-if="pageMetadataState === 'loading'" class="state-spinner" aria-hidden="true" /><IconLanguage v-else aria-hidden="true" /><span><strong>{{ t('panels.pageMetadata') }}</strong><small>{{ labels.pageMetadata }}</small></span></button>
          <button :class="{ complete: coverageResult?.status === 'complete', error: coverageState === 'error', running: Boolean(activeTab?.codeCoverageRecording) || coverageState === 'loading' }" type="button" :aria-label="t('coverage.toolAria', { status: labels.coverage })" :disabled="coverageState === 'loading'" @click="toggleCodeCoverage"><IconProgress v-if="coverageState === 'loading'" class="state-spinner" aria-hidden="true" /><IconCode v-else aria-hidden="true" /><span><strong>{{ t('panels.coverage') }}</strong><small>{{ labels.coverage }}</small></span></button>
          <button :class="{ complete: cpuProfileResult?.status === 'complete', error: cpuProfileState === 'error', running: Boolean(activeTab?.cpuProfileRecording) || cpuProfileState === 'loading' }" type="button" :aria-label="t('cpuProfile.toolAria', { status: labels.cpuProfile })" :disabled="cpuProfileState === 'loading'" @click="toggleCpuProfile"><IconProgress v-if="cpuProfileState === 'loading'" class="state-spinner" aria-hidden="true" /><IconMonitoring v-else aria-hidden="true" /><span><strong>{{ t('shell.pageTools.javascriptCpu') }}</strong><small>{{ labels.cpuProfile }}</small></span></button>
          <button :class="{ error: memoryState === 'error', running: memoryState === 'running' }" type="button" :aria-label="t('memory.toolAria', { status: labels.memory })" :disabled="memoryState === 'running'" @click="toggleMemoryReport"><IconProgress v-if="memoryState === 'running'" class="state-spinner" aria-hidden="true" /><IconError v-else-if="memoryState === 'error'" aria-hidden="true" /><IconMemory v-else aria-hidden="true" /><span><strong>{{ t('panels.memory') }}</strong><small>{{ labels.memory }}</small></span></button>
        </div>
      </section>
      <section aria-labelledby="page-tools-export-title">
        <h3 id="page-tools-export-title">{{ t('shell.pageTools.exportAccount') }}</h3>
        <div class="page-tools-grid">
          <button :class="{ copied: snapshotState === 'copied', error: snapshotState === 'error', running: snapshotState === 'copying' }" type="button" :aria-label="t('shell.pageTools.copySnapshotAria')" :disabled="snapshotState === 'copying'" @click="actions.copyPageSnapshot">
          ...
          <button type="button" :aria-label="labels.pdfExport" :disabled="pdfState === 'saving'" @click="closeAndRun(actions.savePdf)">
          ...
          <button type="button" :aria-label="credentialCount ? t('shell.pageTools.fillPassword') : t('shell.pageTools.noPassword')" :disabled="!credentialStorageAvailable || !credentialCount" @click="closeAndRun(actions.fillSavedPassword)">

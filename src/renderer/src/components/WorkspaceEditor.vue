...
<template>
  <div v-if="open" class="tab-group-editor-overlay" @click.self="close">
    <form class="tab-group-editor workspace-editor" role="dialog" aria-modal="true" aria-labelledby="tab-group-editor-title" @submit.prevent="save">
      <header>
      ...
      <div class="workspace-editor-body">
        ...
        <input id="tab-group-name" v-model="name" type="text" maxlength="80" autocomplete="off" autofocus :disabled="mode === 'edit' && isDefault" />
        ...
        <section v-if="mode === 'create'" class="workspace-storage-section">
          ...
          <label class="workspace-storage-choice">
            <input v-model="storageMode" type="radio" value="scratch" />
            <span><strong>{{ t('workspaceEditor.scratch') }}</strong><small>{{ t('workspaceEditor.scratchDescription') }}</small></span>
          </label>
          <label class="workspace-storage-choice">
            <input v-model="storageMode" type="radio" value="fork-default" />
          ...
          <div v-if="storageMode === 'fork-default'" class="workspace-origin-picker">
            ...
            <p v-if="!originOptions.length">{{ t('workspaceEditor.noOrigins') }}</p>
            <label v-for="origin in originOptions" :key="origin"><input v-model="selectedOrigins" type="checkbox" :value="origin" /><span>{{ origin }}</span></label>
          </div>
        </section>
        ...
        <section v-else class="workspace-storage-section">
          ...
          <div class="workspace-transfer-direction" role="radiogroup" :aria-label="t('workspaceEditor.transferDirection')">
            <label><input v-model="transferDirection" type="radio" value="from-default" /><span>{{ t('workspaceEditor.importDefault') }}</span></label>
            <label><input v-model="transferDirection" type="radio" value="to-default" /><span>{{ t('workspaceEditor.saveDefault') }}</span></label>
          </div>
          ...
          <button class="workspace-transfer-button" type="button" :disabled="storageState === 'saving' || storageState === 'loading' || storageState === 'error'" @click="transferStorage"><IconSwapHoriz aria-hidden="true" /> {{ storageState === 'saving' ? t('workspaceEditor.copying') : transferDirection === 'from-default' ? t('workspaceEditor.importSelected') : t('workspaceEditor.saveSelected') }}</button>
          <output v-if="storageMessage" :class="{ error: storageState === 'error' }" role="status">{{ storageMessage }}</output>
          <div class="workspace-danger-zone"><div><strong>{{ t('workspaceEditor.closePermanently') }}</strong><span>{{ t('workspaceEditor.closeDescription') }}</span></div><button type="button" :disabled="state.allHumanInteractionLocked" :title="state.allHumanInteractionLocked ? t('workspaceEditor.unlockTitle') : undefined" data-lock-protected-tab-close @click="closeWorkspace">{{ t('workspaceEditor.closeWorkspace') }}</button></div>
        </section>
        <output v-if="error" class="workspace-editor-error" role="alert">{{ error }}</output>
      </div>
      <footer><button type="button" @click="close">{{ t('workspaceEditor.cancel') }}</button><button class="primary" type="submit" :disabled="!name.trim()">{{ mode === 'create' ? t('workspaceEditor.create') : t('workspaceEditor.save') }}</button></footer>
    </form>
  </div>
</template>

...
<template>
  <main class="settings-content support-settings" :aria-busy="busy">
    ...
    <div v-else class="support-card commercial-license-card">
      ...
      <button class="secondary-button" type="button" @click="emit('openUrl', 'https://bronom.pages.dev')">{{ t('settings.support.support') }}</button>
    </div>
    <div class="support-alternatives">
      <span>{{ t('settings.support.alternatives') }}</span>
      <button type="button" @click="emit('openUrl', 'https://github.com/Netroforge/bronom/blob/main/LICENSE')">{{ t('settings.support.license') }}</button>
      <button type="button" @click="emit('openUrl', 'https://github.com/Netroforge/bronom/blob/main/CONTRIBUTING.md')">{{ t('settings.support.contributing') }}</button>
      <button type="button" @click="emit('openUrl', 'https://github.com/Netroforge/bronom/issues')">{{ t('settings.support.issue') }}</button>
    </div>
  </main>
</template>

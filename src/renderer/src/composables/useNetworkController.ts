...
export interface NetworkControllerOptions {
  ...
  acceptBrowserState: (state: BrowserState) => void
...
export function useNetworkController(options: NetworkControllerOptions) {
  ...
  async function updatePreservation(event: Event): Promise<void> {
    const tab = options.activeTab.value
    if (!tab) return
    const preserve = (event.currentTarget as HTMLInputElement).checked
    const nextState = await options.browser.setDiagnosticLogPreservation(tab.id, preserve)
    if (options.activeTab.value?.id === tab.id) options.acceptBrowserState(nextState)
  }
  ...
  async function addRouteFromDraft(): Promise<void> {
    ...
    try {
      ...
      const nextState = await options.browser.getState()
      if (sequence !== routeMutationSequence || !isCurrent(tab.id, expectedGeneration)) return
      routes.value = nextRoutes
      options.acceptBrowserState(nextState)
      resetRouteDraft()
      routeState.value = 'ready'
    } catch (cause) {
      if (sequence !== routeMutationSequence || !isCurrent(tab.id, expectedGeneration)) return
  ...
  async function clearRoutes(): Promise<void> {
    ...
    routeError.value = ''
    try {
      const nextState = await options.browser.clearNetworkRoutes(tab.id)
      if (sequence !== routeMutationSequence || !isCurrent(tab.id, expectedGeneration)) return
      options.acceptBrowserState(nextState)
      routes.value = []
      routeState.value = 'ready'

...
Bronom depends on and bundles third-party open-source software. Each dependency remains under its own license; the Apache-2.0 license for Bronom does not replace those terms. Complete dependency names, versions, and integrity hashes are recorded in `package-lock.json`.

Notable code and assets compiled into the desktop application include:

- **axe-core 4.12.1**, Copyright Deque Systems, Inc. and contributors, licensed under the Mozilla Public License 2.0. Source: <https://github.com/dequelabs/axe-core/tree/v4.12.1>. The complete license and upstream third-party notices are distributed in `node_modules/axe-core/LICENSE` and `node_modules/axe-core/LICENSE-3RD-PARTY.txt`.

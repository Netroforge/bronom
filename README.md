...
Use the database button in the website toolbar to inspect, filter, add, edit, or delete the active tab's local storage, session storage, and cookies. The **IndexedDB** tab lists top-level-origin databases, object-store schemas, indexes, counts, and bounded record previews without opening Developer Tools; it is deliberately read-only. The **Offline** tab shows bounded web app manifest fields, Chromium installability findings, service-worker registrations, and paginated Cache Storage request metadata without returning raw manifest source or cached response bodies. The **Changes** tab can save an in-memory baseline and show which local-storage, session-storage, or cookie keys were added, updated, or removed after reproducing an issue. Local storage, workers, caches, and cookies are shared by origin inside the current workspace and isolated from other workspaces; session storage belongs only to the selected tab. Bronom displays sizes, bounds long previews, and keeps HttpOnly cookie values protected from both the human panel and MCP storage tools.
...
Default is Bronom's shared durable Chromium profile. Every non-Default workspace receives its own persistent Electron partition for cookies, local storage, cache, service workers, and related website state. A new workspace can start clean or fork selected origins from Default. Later, **Import from Default** or **Save to Default** explicitly merges selected cookies and local storage without linking future changes. Bronom rejects overlapping storage copies and prevents a workspace from being renamed, archived, or deleted while its profile is being copied. MCP clients use the matching `browser_workspaces` `create`, `list-origins`, `import-default`, and `save-default` actions while remaining forbidden from browsing Default. Bookmarks, visit history, download records, and Bronom's remembered permission decisions remain application-wide.
